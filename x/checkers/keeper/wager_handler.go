...
func (k *Keeper) MustPayWinnings(ctx sdk.Context, storedGame *types.StoredGame) {
	...
	winnigns := storedGame.GetWagerCoin()
	if storedGame.MoveCount == 0 {
		panic(types.ErrNothingToPay.Error())
	} else if 1 < storedGame.MoveCount {
		winnigns = winnigns.Add(winnigns)
	}
	err = k.bank.SendCoinsFromModuleToAccount(ctx, types.ModuleName, winnerAddress, sdk.NewCoins(winnigns))
	if err != nil {
